...
public static class FactoriesActions
{
    ...
    //         int baseAmt = Piece.factory_payout[type];
    //         if (baseAmt == 0 && AuxPayout == 0) continue;

    //         // Flags for scaling
    //         bool roundMul = Piece.factory_isRoundMultiplier[type];

    //         bool group = Piece.groupFactory_enabled[type];

    //         int groupAmt = Piece.groupFactory_require[type];
